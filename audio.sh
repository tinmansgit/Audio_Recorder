...
record() {
	...
	ffmpeg -f jack -i "ffmpeg" "$outfile" > /dev/null 2>&1 &
    proc_pid=$!
    echo "${proc_pid}" > "${PIDFILE}"
    rofi -theme-str "window {width: 400;}" -e "Audio recording started..."
    ...
    if [[ -f "${PIDFILE}" ]]; then
        ...
        rofi -theme-str "window {width: 400;}" -e "Audio recording ended!"
